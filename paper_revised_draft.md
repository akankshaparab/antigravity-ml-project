...
This research addresses this gap by investigating the structure of query embedding spaces across both academic benchmarks (384D) and production environments (768D). Specifically, the problem is formalized by addressing four core questions:
1. **Intrinsic Dimensionality**: How much of the variance in query embeddings is captured by a small number of principal components, and is the effective dimensionality of the space significantly lower than its nominal dimension?
...
**Embedding Models**: For the baseline experiments, natural language queries were encoded using the **BAAI/bge-small-en-v1.5** model [2], which yields 384-dimensional dense vectors. For the production environment experiments, queries were encoded using the **BAAI/bge-base-en-v1.5** model, which yields 768-dimensional dense vectors to accommodate the broader vocabulary of live enterprise traffic.
